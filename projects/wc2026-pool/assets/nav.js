...
(function(){
  ...
  const NAV_T = {
    es:{home:'Inicio',teams:'Clasificación',calendar:'Calendario',pool:'Quiniela',modelo:'Modelo',sedes:'Sedes',squads:'Plantillas'},
    en:{home:'Home',teams:'Standings',calendar:'Calendar',pool:'Pool',modelo:'Model',sedes:'Venues',squads:'Squads'},
    tr:{home:'Ana sayfa',teams:'Puan durumu',calendar:'Takvim',pool:'Tahmin',modelo:'Model',sedes:'Sahalar',squads:'Kadrolar'}
  };
  const NAV_S = {
    es:{home:'Inicio',teams:'Clasif.',calendar:'Calend.',pool:'Quiniela',modelo:'Modelo',sedes:'Sedes',squads:'Plantel'},
    en:{home:'Home',teams:'Standings',calendar:'Calendar',pool:'Pool',modelo:'Model',sedes:'Venues',squads:'Squads'},
    tr:{home:'Ana',teams:'Puan',calendar:'Takvim',pool:'Tahmin',modelo:'Model',sedes:'Sahalar',squads:'Kadro'}
  };
  ...
  const LINKS = [
    ...
    {id:'modelo',   label:tn.modelo,   short:ts.modelo,   href:'modelo.html',     icon:'📈', topOnly:true},
  ...
  const CRUMB_BY_HREF = {'index.html':tn.home,'teams.html':tn.teams,'calendar.html':tn.calendar,'pool.html':tn.pool,'modelo.html':tn.modelo,'sedes.html':tn.sedes,'plantillas.html':tn.squads};
  ...
  const crumbsHTML = (P.crumbs&&P.crumbs.length)
    ? `<nav class="sn-crumbs" aria-label="Breadcrumb">`+P.crumbs.map((c,i)=>{
        const sep=i?'<span class="sep" aria-hidden="true">›</span>':'';
        return sep+(c.href?`<a href="${c.href}">${c.label}</a>`:`<span class="cur" aria-current="page">${c.label}</span>`);
      }).join('')+`</nav>` : '';
  const topHTML = `
  ...
  const tabs=LINKS.filter(l=>!l.topOnly).map(l=>`<a class="sn-tab ${P.active===l.id?'active':''}" href="${l.href}"><span class="ic" aria-hidden="true">${l.icon}</span>${l.short||l.label}</a>`).join('');
  ...
  // floating language toggle — on EVERY page now (was Home-only → language got "trapped").
  {
    ...
    const langHTML=['EN','ES','TR'].map(L=>`<button data-l="${L.toLowerCase()}" aria-pressed="${lang===L.toLowerCase()}" class="${lang===L.toLowerCase()?'on':''}">${L}</button>`).join('');
    document.body.insertAdjacentHTML('beforeend', `<div class="sn-lang" role="group" aria-label="Idioma">${langHTML}</div>`);
    document.querySelector('.sn-lang').addEventListener('click', e=>{ const b=e.target.closest('button'); if(!b) return; localStorage.setItem('wc2026-lang', b.dataset.l); localStorage.removeItem('wc26-lang'); location.reload(); });
  }
